...
_ACTIVITY_COLUMNS = (
    "activity_id", "parent_activity_id", "session_index", "source", "file_hash",
    "garmin_activity_id", "sport", "sub_sport", "name",
    "start_time_utc", "start_time_local", "tz_offset_seconds",
    "total_timer_time_s", "total_elapsed_time_s", "total_distance_m",
    "total_ascent_m", "total_descent_m", "total_calories",
    ...
    "aerobic_training_effect", "anaerobic_training_effect", "avg_temperature_c",
    "pool_length_m", "total_strokes", "num_laps",
    "device_product", "device_serial", "start_lat", "start_lon",
    "ingested_at", "extra",
)

_LAP_COLUMNS = (
    "activity_id", "lap_index", "start_time_utc", "total_timer_time_s",
    "total_distance_m", "avg_speed_mps", "max_speed_mps",
    "avg_heart_rate", "max_heart_rate", "avg_cadence",
    "avg_power_w", "normalized_power_w", "total_ascent_m", "total_descent_m",
    "total_calories", "intensity", "lap_trigger",
)

_RECORD_COLUMNS = (
    "activity_id", "ts", "elapsed_s", "lap_index",
    "lat", "lon", "altitude_m", "distance_m", "speed_mps",
    "heart_rate", "cadence", "power_w", "temperature_c", "grade",
    "vertical_oscillation_mm", "vertical_ratio", "stance_time_ms",
    "stance_time_percent", "step_length_mm", "left_right_balance",
    "respiration_rate", "accumulated_power_w", "extra",
)
    ...
    *,
    ...
    try:
        ...
        for table in ("records", "laps", "activities"):
            conn.execute(
                f"DELETE FROM {table} WHERE activity_id IN "
                f"({', '.join('?' * len(activity_ids))})",
                activity_ids,
            )
        conn.execute(
